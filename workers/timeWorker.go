...
func NewTimeWorker(db *database.Client, bc *blockchain.Client, st *time.Time) *TimeWorker {
	return &TimeWorker{
		database:   db,
		blockchain: bc,
		startTime:  st,
...
func (cw *TimeWorker) Start() {
	interval := configs.Envs.AuditTimeout

	ticker := time.NewTicker(time.Duration(interval) * time.Millisecond)
	defer ticker.Stop()

	log.Println("Time worker started with interval:", interval, "milliseconds")
...
func (cw *TimeWorker) performTest() {
...
func (cw *TimeWorker) Test(blockNumber uint64) error {
	...
	// Create or open Excel file
	fileName = fmt.Sprintf("D:\\Studia_mgr\\Praca_magisterska\\Results\\getById_checkpoint_results_%s.xlsx", time.Now().Format("20060102_150405"))
	if _, err := os.Stat(fileName); os.IsNotExist(err) {
		f = excelize.NewFile()
		f.SetSheetName("Sheet1", "Results")
		f.SetCellValue("Results", "A1", "Timestamp")
