...
# =========================
FILES = [
    Path("FullAudit/audit_results_Full_1000.xlsx"),
    Path("FullAudit/audit_results_Full_3000.xlsx"),
    Path("FullAudit/audit_results_Full_5000.xlsx"),
]
SHEET_NAME = "Results"
COL_DB = "Db duration"
COL_BC = "Blockchain duration"

OUTPUT_DIR = Path("FullAudit_plots_combined")
...
def parse_duration(val):
    """'123.4ms' -> 123.4 ; '1.2s' -> 1200.0 ; liczby -> ms"""
...
def extract_blocks_label(path: Path) -> str:
    """Extracts the last number from filename (e.g. ..._3000.xlsx -> '3000')"""
...
def make_multi_boxplot(values_list: list[np.ndarray],
    ...
    """
    Draws one plot with multiple boxplots (each from one file).
    ...
    if title:
        plt.title(title)
    plt.ylabel(ylabel)
    plt.xlabel(x_label)   # <-- podpis osi X ustawiany osobno
    plt.legend()
    plt.tight_layout()
...
# MAIN
# =========================
xlabels = [extract_blocks_label(p) for p in FILES]

# MongoDB plot
mongo_values = [load_ms_from_file(p, COL_DB) for p in FILES]
make_multi_boxplot(
    mongo_values,
    x_tick_labels=xlabels,
    ylabel="Duration [ms]",
    x_label="Number of documents stored in MongoDB database",
    out_path=OUTPUT_DIR / "mongodb_3boxplots"
)
...
make_multi_boxplot(
    ...
    x_tick_labels=xlabels,
    ylabel="Duration [ms]",
    x_label="Number of cryptographic hashes stored in the Ethereum network",
    out_path=OUTPUT_DIR / "ethereum_3boxplots"
)

print("\n✅ Done. Two plots saved in:", OUTPUT_DIR.as_posix())
